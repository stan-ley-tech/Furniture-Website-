...
- **Deployment topology:** Supabase (DB) + Render (backend, via `render.yaml` at the repo root) + Vercel (frontend). `render.yaml` declares `SUPABASE_URL`, `SUPABASE_KEY`, `FRONTEND_ORIGIN` as `sync: false` (set manually in the Render dashboard, not stored in the repo). The frontend's `PUBLIC_API_BASE_URL` (Vercel env var) must point at the Render backend URL, and the backend's `FRONTEND_ORIGIN` must point back at the Vercel URL — see README "Deployment".
